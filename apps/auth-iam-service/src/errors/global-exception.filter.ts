...
export class GlobalExceptionFilter implements ExceptionFilter {
    ...
    const req = http.getRequest<AuthIamRequest>();
  ...
  private getErrorBody(exception: unknown, statusCode: number): Required<AuthIamErrorBody> {
    if (exception instanceof HttpException) {
      const response = exception.getResponse();
      if (typeof response === "object" && response !== null) {
        const body = response as AuthIamErrorBody;
        return {
  ...
  private logError(exception: unknown, req: AuthIamRequest, statusCode: number) {
