...
export class GlobalExceptionFilter implements ExceptionFilter {
  private readonly config = getAppConfig();
    ...
    const req = http.getRequest<AdminBffRequest>();
  ...
  private getErrorBody(exception: unknown, statusCode: number): Required<AdminBffErrorBody> {
    if (exception instanceof HttpException) {
      const response = exception.getResponse();
      if (typeof response === "object" && response !== null) {
        const body = response as AdminBffErrorBody;
        return {
  ...
  private logError(exception: unknown, req: AdminBffRequest, statusCode: number) {
