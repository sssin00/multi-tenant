...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  private getErrorBody(exception: unknown, statusCode: number): Required<AuditErrorBody> {
    if (exception instanceof HttpException) {
      const response = exception.getResponse();
      if (typeof response === "object" && response !== null) {
        const body = response as AuditErrorBody;
        return {
  ...
  private logError(exception: unknown, req: AuditRequest, statusCode: number) {
