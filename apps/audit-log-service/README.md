...
외부 조회 요청은 `gateway-service` -> `admin-bff-service` -> `audit-log-service` 순서로 처리합니다. 감사 로그 저장은 각 서비스의 outbox 이벤트를 audit-log-service consumer가 소비하는 방식으로 처리하며, `eventId` 기준으로 중복 저장을 방지합니다.
...
- `AUDIT_INTERNAL_AUTH_ALLOWED_SERVICES`: 감사 로그 내부 조회 API를 호출할 수 있는 서비스 목록입니다. 기본값은 `admin-bff-service`입니다.
