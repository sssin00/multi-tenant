...
export class GlobalExceptionFilter implements ExceptionFilter {
  private readonly config = getAppConfig();
    ...
    const req = http.getRequest<OutboxRequest>();
  ...
  private getErrorBody(exception: unknown, statusCode: number): Required<OutboxErrorBody> {
    if (exception instanceof HttpException) {
      const response = exception.getResponse();
      if (typeof response === "object" && response !== null) {
        const body = response as OutboxErrorBody;
        return {
  ...
  private logError(exception: unknown, req: OutboxRequest, statusCode: number) {
