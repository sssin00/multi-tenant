...
export class HealthService {
  ...
  private checkConfig(config: AppConfig): ReadinessCheck {
    if (!config.port || config.port <= 0) {
      return { status: "failed", message: "TENANT_PORT must be a positive number" };
    }
  ...
  private checkInternalAuth(config: AppConfig): ReadinessCheck {
    ...
    if (!config.internalAuth.secret) {
      return { status: "failed", message: "TENANT_INTERNAL_AUTH_SECRET is required when internal auth is enabled" };
    }

    if (config.internalAuth.secret.length < 32) {
      return { status: "failed", message: "TENANT_INTERNAL_AUTH_SECRET must be at least 32 characters" };
    }

    if (config.internalAuth.allowedServices.length === 0) {
      return { status: "failed", message: "TENANT_INTERNAL_AUTH_ALLOWED_SERVICES must include at least one service" };
    }

    if (config.internalAuth.timestampSkewSeconds <= 0) {
      return { status: "failed", message: "TENANT_INTERNAL_AUTH_TIMESTAMP_SKEW_SECONDS must be positive" };
    }
  ...
  private checkSecurity(config: AppConfig): ReadinessCheck {
    if (config.env === "prod" && config.cors.allowedOrigins.length === 0) {
      return { status: "failed", message: "TENANT_CORS_ALLOWED_ORIGINS is required in prod" };
    }
