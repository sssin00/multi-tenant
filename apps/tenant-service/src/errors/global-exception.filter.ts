...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  private getErrorBody(exception: unknown, statusCode: number): Required<TenantErrorBody> {
    if (exception instanceof HttpException) {
      const response = exception.getResponse();
      if (typeof response === "object" && response !== null) {
        const body = response as TenantErrorBody;
        return {
  ...
  private logError(exception: unknown, req: TenantRequest, statusCode: number) {
