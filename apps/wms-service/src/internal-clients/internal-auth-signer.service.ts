...
export type InternalAuthTarget = "auth-iam-service" | "tenant-service";
...
export class InternalAuthSignerService {
  ...
  sign(command: InternalAuthSignCommand): Record<string, string> {
    ...
    const payload = [command.method.toUpperCase(), command.originalUrl, timestamp, command.requestId, bodyHash].join("\n");
