...
export class GlobalExceptionFilter implements ExceptionFilter {
    ...
    const req = http.getRequest<WmsRequest>();
  ...
  private getErrorBody(exception: unknown, statusCode: number): Required<WmsErrorBody> {
    if (exception instanceof HttpException) {
      const response = exception.getResponse();
      if (typeof response === "object" && response !== null) {
        const body = response as WmsErrorBody;
        return {
  ...
  private logError(exception: unknown, req: WmsRequest, statusCode: number) {
