...
## gateway-service/auth-iam-service/tenant-service/admin-bff-service 1차 배포 흐름

현재 CDK는 `gateway-service`, `auth-iam-service`, `tenant-service`, `admin-bff-service` 배포를 위한 최소 dev 인프라를 생성합니다.
...
첫 CDK 배포는 ECR repository를 먼저 만들 수 있도록 `gatewayDesiredCount=0`, `authDesiredCount=0`, `tenantDesiredCount=0`, `adminBffDesiredCount=0`을 기본값으로 사용합니다.
...
같은 CDK 스택 안의 auth-iam-service, tenant-service, admin-bff-service 리소스 및 task definition context도 함께 갱신되므로, 해당 서비스의 desired count와 image tag는 GitHub Environment variable에 반드시 명시해야 합니다.
Auth/IAM 실행 배포는 `deploy-auth-iam-service.yml`, Tenant 실행 배포는 `deploy-tenant-service.yml`, Admin BFF 실행 배포는 `deploy-admin-bff-service.yml`에서 별도로 처리합니다.

### 배포 workflow 요약

네 workflow는 모두 같은 CDK 스택(`multi-tenant-{env}-gateway-service-stack`)을 갱신합니다.
...
| --- | --- | --- | --- | --- |
| `deploy-gateway-service.yml` | gateway-service | `GATEWAY_DESIRED_COUNT=1` | `AUTH_DESIRED_COUNT`, `AUTH_IMAGE_TAG`, `TENANT_DESIRED_COUNT`, `TENANT_IMAGE_TAG`, `ADMIN_BFF_DESIRED_COUNT`, `ADMIN_BFF_IMAGE_TAG` 명시 필수 | `GATEWAY_JWT_SECRET_ARN` |
| `deploy-auth-iam-service.yml` | auth-iam-service | `AUTH_DESIRED_COUNT=1` | `GATEWAY_DESIRED_COUNT`, `GATEWAY_IMAGE_TAG`, `TENANT_DESIRED_COUNT`, `TENANT_IMAGE_TAG`, `ADMIN_BFF_DESIRED_COUNT`, `ADMIN_BFF_IMAGE_TAG` 명시 필수 | `AUTH_DATABASE_URL_SECRET_ARN`, JWT secret/key, `AUTH_INTERNAL_AUTH_SECRET_ARN` |
| `deploy-tenant-service.yml` | tenant-service | `TENANT_DESIRED_COUNT=1` | `GATEWAY_DESIRED_COUNT`, `GATEWAY_IMAGE_TAG`, `AUTH_DESIRED_COUNT`, `AUTH_IMAGE_TAG`, `ADMIN_BFF_DESIRED_COUNT`, `ADMIN_BFF_IMAGE_TAG` 명시 필수 | `TENANT_DATABASE_URL_SECRET_ARN`, `TENANT_INTERNAL_AUTH_SECRET_ARN` |
| `deploy-admin-bff-service.yml` | admin-bff-service | `ADMIN_BFF_DESIRED_COUNT=1` | `GATEWAY_DESIRED_COUNT`, `GATEWAY_IMAGE_TAG`, `AUTH_DESIRED_COUNT`, `AUTH_IMAGE_TAG`, `TENANT_DESIRED_COUNT`, `TENANT_IMAGE_TAG` 명시 필수 | `ADMIN_BFF_AUTH_INTERNAL_AUTH_SECRET_ARN`, `ADMIN_BFF_TENANT_INTERNAL_AUTH_SECRET_ARN` |

중요: 네 workflow 모두 같은 스택을 다시 배포합니다. 비대상 서비스 값이 비어 있으면 workflow가 실패하도록 막아 두었으므로, 처음 인프라만 만들 때도 명시적으로 `0`과 사용할 image tag를 넣어야 합니다.
예를 들어 tenant만 새로 배포하면서 gateway, auth, admin-bff를 계속 1개씩 유지하려면 `GATEWAY_DESIRED_COUNT=1`, `GATEWAY_IMAGE_TAG={current_gateway_tag}`, `AUTH_DESIRED_COUNT=1`, `AUTH_IMAGE_TAG={current_auth_tag}`, `ADMIN_BFF_DESIRED_COUNT=1`, `ADMIN_BFF_IMAGE_TAG={current_admin_bff_tag}`를 함께 설정합니다. 해당 서비스 desired count가 0보다 크면 그 서비스의 runtime secret ARN도 같이 설정해야 합니다.
...
2. CDK typecheck
3. CDK deploy with target `gatewayDesiredCount=0`, and explicitly provided auth/tenant/admin-bff desired counts
4. ECR repository URI 조회
5. gateway-service Docker image build/push
6. CDK deploy with target gateway desired count, and explicitly provided auth/tenant/admin-bff desired counts
...
| `ADMIN_BFF_AUDIT_INTERNAL_AUTH_SECRET_ARN` | `AUDIT_LOG_SERVICE_URL`이 설정된 경우 audit-log-service 내부 API를 호출할 때 사용할 secret ARN |
...
같은 CDK 스택을 갱신하므로 gateway-service, auth-iam-service, admin-bff-service의 desired count와 image tag를 GitHub Environment variable에 반드시 명시해야 합니다.
...
같은 CDK 스택을 갱신하므로 gateway-service, auth-iam-service, tenant-service의 desired count와 image tag를 GitHub Environment variable에 반드시 명시해야 합니다.
...
`AUDIT_LOG_SERVICE_URL`을 설정하면 `ADMIN_BFF_AUDIT_INTERNAL_AUTH_SECRET_ARN`도 필요합니다.
