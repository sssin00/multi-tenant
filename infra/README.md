...
## gateway-service/auth-iam-service/tenant-service/admin-bff-service/user-bff-service/wms-service/audit-log-service/outbox-relay-service 1차 배포 흐름

현재 CDK는 `gateway-service`, `auth-iam-service`, `tenant-service`, `admin-bff-service`, `user-bff-service`, `wms-service`, `audit-log-service`, `outbox-relay-service` 배포를 위한 최소 dev 인프라를 생성합니다.
...
첫 CDK 배포는 ECR repository를 먼저 만들 수 있도록 `gatewayDesiredCount=0`, `authDesiredCount=0`, `tenantDesiredCount=0`, `adminBffDesiredCount=0`, `userBffDesiredCount=0`, `wmsDesiredCount=0`, `auditDesiredCount=0`, `outboxDesiredCount=0`을 기본값으로 사용합니다.
...
user-bff-service도 ALB target으로 직접 노출하지 않습니다. gateway-service가 Cloud Map 내부 DNS와 app base path를 포함한 `http://user-bff-service.{envName}.multi-tenant.local:3000/api/app`으로 `/api/app/**` upstream을 호출합니다. User BFF는 Auth/IAM, tenant-service, wms-service를 내부 호출해 app shell, navigation, WMS 화면 조회 API를 조합합니다. User BFF app shell 감사는 `userBffAppAuditPublisherType=eventbridge`와 `userBffAuditEventBridgeBusName`을 지정하면 EventBridge로 직접 발행하고, 기본 context인 `internal-api`에서는 기존 audit-log-service 내부 POST fallback을 사용합니다.
audit-log-service도 ALB target으로 직접 노출하지 않습니다. 감사 로그 저장은 각 서비스의 outbox event 발행과 audit-log-service consumer로 처리하고, admin-bff-service만 Cloud Map 내부 DNS인 `http://audit-log-service.{envName}.multi-tenant.local:3000`으로 감사 로그 조회 API를 호출합니다. `auditDesiredCount=0`이면 admin-bff-service의 `AUDIT_LOG_SERVICE_URL`은 빈 값으로 주입되며, `auditDesiredCount>0`이거나 `auditLogServiceUrl` override를 지정한 경우 조회용 audit internal auth secret ARN이 필요합니다. User BFF가 `internal-api` 감사 fallback을 쓰는 환경도 audit internal auth secret이 필요합니다.
...
docker buildx build \
  --platform linux/amd64 \
  -f apps/user-bff-service/Dockerfile \
  -t {UserBffServiceRepositoryUri}:latest \
  --push .

docker buildx build \
  --platform linux/amd64 \
  -f apps/wms-service/Dockerfile \
  -t {WmsServiceRepositoryUri}:latest \
  --push .
...
pnpm --filter @multi-tenant/infra-cdk deploy \
  ...
  -c userBffImageTag=latest \
  -c wmsImageTag=latest \
...
### Optional User BFF context secrets and audit publisher

아래 값은 모든 workflow에서 CDK task definition context로 전달할 수 있습니다. User BFF app shell은 auth-iam-service와 tenant-service를 호출하고, WMS 화면 조회 API는 wms-service를 호출합니다. app audit internal-api fallback을 사용할 때는 audit-log-service secret이 필요합니다.

| Name | Description |
| --- | --- |
| `USER_BFF_AUTH_INTERNAL_AUTH_SECRET_ARN` | User BFF가 auth-iam-service 내부 API를 호출할 때 사용할 secret ARN |
| `USER_BFF_TENANT_INTERNAL_AUTH_SECRET_ARN` | User BFF가 tenant-service 내부 API를 호출할 때 사용할 secret ARN |
| `USER_BFF_WMS_INTERNAL_AUTH_SECRET_ARN` | User BFF가 wms-service 내부 WMS 조회 API를 호출할 때 사용할 secret ARN |
| `USER_BFF_AUDIT_INTERNAL_AUTH_SECRET_ARN` | `userBffAppAuditPublisherType=internal-api`일 때 User BFF가 audit-log-service fallback 기록 API를 호출할 때 사용할 secret ARN |

User BFF app audit publisher 기준:

- `userBffAppAuditPublisherType`은 `eventbridge`, `internal-api`, `disabled`를 허용합니다. CDK 기본값은 기존 배포 호환을 위해 `internal-api`입니다.
- `userBffAppAuditPublisherType=eventbridge`이면 `userBffAuditEventBridgeBusName`이 필요하며, User BFF task role에 해당 bus의 `events:PutEvents` 권한이 추가됩니다.
- 같은 설정에서 CDK는 `userBff.appContext.loaded`, `userBff.navigation.loaded` 이벤트를 audit event SQS queue로 전달하는 EventBridge rule을 생성합니다. audit-log-service의 기존 SQS consumer가 이 queue를 polling해 append-only 저장합니다.
- EventBridge 모드는 User BFF task env에 `USER_BFF_APP_AUDIT_PUBLISHER_TYPE`, `USER_BFF_AUDIT_EVENTBRIDGE_BUS_NAME`, `USER_BFF_AUDIT_EVENT_SOURCE_PREFIX={project}.{env}`를 주입합니다.
